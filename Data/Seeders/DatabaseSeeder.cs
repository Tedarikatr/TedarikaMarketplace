        ...
        public async Task SeedAsync()
        {
            try
            {
                ...
                if (!await _context.Regions.AnyAsync())
                {
                    ...
                    {
                        // Region - EU
                        var region = await _context.Regions.FirstOrDefaultAsync(c => c.Code == "EU");
                        if (region != null)
                        {
                            var existingStoreRegion = await _context.StoreLocationRegions
                                .FirstOrDefaultAsync(r => r.RegionId == region.Id && r.StoreId == store.Id);

                            if (existingStoreRegion == null)
                            {
                                var storeRegion = new StoreLocationRegion
                                {
                                    StoreId = store.Id,
                                    RegionId = region.Id,
                                ...
                                await _context.StoreLocationRegions.AddAsync(storeRegion);
                                _logger.LogInformation("StoreLocationRegion başarıyla eklendi.");
                            }
                        }
                        ...
                        if (turkey != null)
                        {
                            var existingCountry = await _context.StoreLocationCountries
                                .FirstOrDefaultAsync(sc => sc.StoreId == store.Id && sc.CountryId == turkey.Id);
                            if (existingCountry == null)
                            {
                                var storeCountry = new StoreLocationCountry
                                {
                                    StoreId = store.Id,
                                    CountryId = turkey.Id,
                                    CountryName = turkey.Name,
                                    DeliveryTimeFrame = 2,
                                    IsActive = true
                                };
                                await _context.StoreLocationCountries.AddAsync(storeCountry);
                                _logger.LogInformation("StoreLocationCountry başarıyla eklendi.");
                            }

                            // Province - İzmir
                            var izmir = await _context.Provinces
                                .FirstOrDefaultAsync(p => p.CountryId == turkey.Id && p.Name == "İzmir");

                            if (izmir != null)
                            {
                                var existingProvince = await _context.StoreLocationProvinces
                                    .FirstOrDefaultAsync(p => p.ProvinceId == izmir.Id && p.StoreId == store.Id);

                                if (existingProvince == null)
                                {
                                    var storeProvince = new StoreLocationProvince
                                    {
                                        StoreId = store.Id,
                                        ProvinceId = izmir.Id,
                                        ProvinceName = izmir.Name,
                                        DeliveryTimeFrame = 1,
                                        IsActive = true
                                    };
                                    await _context.StoreLocationProvinces.AddAsync(storeProvince);
                                    _logger.LogInformation("StoreLocationProvince başarıyla eklendi.");
                                }

                                // Districts
                                var existingDistrictIds = await _context.StoreLocationDistricts
                                    .Where(d => d.StoreId == store.Id)
                                    .Select(d => d.DistrictId)
                                    .ToListAsync();

                                var izmirDistricts = await _context.Districts
                                    .Where(d => d.ProvinceId == izmir.Id)
                                    .ToListAsync();

                                izmirDistricts = izmirDistricts
                                    .Where(d => !existingDistrictIds.Contains(d.Id))
                                    .ToList();

                                if (izmirDistricts.Any())
                                {
                                    var storeDistricts = izmirDistricts.Select(d => new StoreLocationDistrict
                                    {
                                        StoreId = store.Id,
                                        DistrictId = d.Id,
                                        DistrictName = d.Name,
                                    ...
                                    await _context.StoreLocationDistricts.AddRangeAsync(storeDistricts);
